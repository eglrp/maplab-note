...
void createLocalizationSummaryMapFromLandmarkList(
  ...
  G_landmark_position.resize(Eigen::NoChange, landmark_ids.size());//所有的地图点的坐标
  ...
  for (size_t landmark_index = 0; landmark_index < landmark_ids.size();
       ++landmark_index)
  {//遍历所有的地图点
    const vi_map::LandmarkId& landmark_id = landmark_ids[landmark_index];//地图点id
    const vi_map::Landmark& landmark = map.getLandmark(landmark_id);//地图点
    G_landmark_position.col(landmark_index) =//地图点坐标
        map.getLandmark_G_p_fi(landmark_id);

    const std::vector<vi_map::KeypointIdentifier>& landmark_observations =
        landmark.getObservations();//所有地图点的观测信息，都是第几个节点第几个相机去观测到的
    observations.insert(
    ...
    for (size_t i = 0; i < landmark_observations.size(); ++i) {
      observation_to_landmark.push_back(landmark_index);//方便索引
    }
  }
  ...
  std::unordered_map<vi_map::VisualFrameIdentifier, int> frame_id_to_index;//观测到这个地图点的是哪个相机的第几帧,它首次看到的第一个地图点
  int observer_index = 0;
  for (size_t observation_index = 0; observation_index < observations.size();//所有地图点的观测信息
       ++observation_index)
  {
    // We store the observer index for covisibility graph based filtering.
    const vi_map::KeypointIdentifier& observation =//当前的观测数据结构
        observations[observation_index];
    std::unordered_map<vi_map::VisualFrameIdentifier, int>::const_iterator it =
        frame_id_to_index.find(observation.frame_id);//
    if (it == frame_id_to_index.end()) {//这个啥意思？每一个相机的每一个帧只存储一个它对应的地图点？
    ...
    //如果在之后的地图点找到这个节点的这个相机，那么就通过observer_indices索引上
    //第几个观测信息，还观测到了谁
