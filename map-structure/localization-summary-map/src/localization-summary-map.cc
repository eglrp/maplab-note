    ...
    vi_map::LandmarkIdList* landmark_ids) const
{
    ...
    for (const std::pair<vi_map::LandmarkId, int>& value : landmark_id_to_landmark_index_) //遍历所有的
    {
        CHECK_LT(value.second, static_cast<int>(landmark_ids->size()));
        (*landmark_ids)[value.second] = value.first;//value是landmark_ids的索引id,key是实际的LandmarkId
    }
}
