...
void Landmark::addObservation(
  ...
  if (!appearances_.empty()) {//应该是出现过就设成-1
  ...
  return false;
            ...
            predicate)
    {
        ...
        while (observation_iterator != observations_.end()) {
            ...
            //      {
            //        return observation.frame_id.vertex_id == vertex_id;//这个观测的节点id是否等于当前id？
            //      };
            if (predicate(*observation_iterator))//如果等于当前这个节点，清除观测信息
            {
...
void Landmark::removeAllObservationsOfVertex(
    ...
    {
    //定义了一个lambda表达式函数，bool是它的返回值，输入是KeypointIdentifier
  std::function<bool(const KeypointIdentifier& observation)> // NOLINT
      predicate =
      [&](const KeypointIdentifier& observation)
      {
