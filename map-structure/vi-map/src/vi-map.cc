...
pose_graph::Edge::EdgeType VIMap::getGraphTraversalEdgeType(
  ...
  //得到当前任务边的类型,比如vio系统，它的边应该就是kViwls（这个还需要再验证一下,应该在前端有设置这个边的属性）
...
void VIMap::addNewLandmark(
  ...
  CHECK_EQ(0u, landmark.numberOfObserverVertices())
      << "The new landmark shouldn't contain any backlinks as this may cause "
        ...
        //LandmarkStore中进行地图点的增加，这个数据结构只储存这个节点新看到的地图点信息
  getVertex(keypoint_and_store_vertex_id).getLandmarks().addLandmark(landmark);

  const vi_map::LandmarkId& landmark_id = landmark.id();
  landmark_index.addLandmarkAndVertexReference(//landmark_index的LandmarkToVertexMap中加入pair<地图点的id，节点的id>
      landmark_id, keypoint_and_store_vertex_id);
  CHECK(hasLandmark(landmark_id));
        //对地图点进行特征点的关联，即更新节点的观测以及地图点的2d观测
        associateKeypointWithExistingLandmark(
      keypoint_and_store_vertex_id, frame_index, keypoint_index, landmark_id);
}

void VIMap::addNewLandmark(
    const LandmarkId& predefined_landmark_id,//predefined_landmark_id在first_observation也有
    const KeypointIdentifier& first_observation) //当第一次观测时，需要加入地图点
    {
    //
...
//对地图点进行特征点的关联
            ...
            unsigned int keypoint_index, const vi_map::LandmarkId& landmark_id)
    {
        ...
        // Update vertex.
        //如果观测到了这个地图点，就要添加它的观测
        //也就是对vertex里的observed_landmark_ids_[frame_idx][keypoint_idx] = landmark_id;
        vertex.setObservedLandmarkId(frame_index, keypoint_index, landmark_id);

        // Update landmark.
        //这里路标点一样需要在它的observations_将这个观测的2d信息加入
...
//被合并的节点中首次观测地图点合并进这个要合并的节点的首次观测地图点中
    void VIMap::moveLandmarksToOtherVertex(
            const pose_graph::VertexId& vertex_id_from,//被合并的节点
            const pose_graph::VertexId& vertex_id_to)//要合并的节点
    {
        ...
        for (const vi_map::Landmark& landmark : landmark_store_from)
         {//遍历第一次被观测到是在这个被合并节点中的路标点
            ...
            const Eigen::Vector3d& p_G_fi = getLandmark_G_p_fi(landmark_id);//这个地图点在世界坐标系下的位置
            const vi_map::MissionBaseFrame& mission_baseframe_to =//这个任务的基准帧
                    getMissionBaseFrame(getMissionForVertex(vertex_id_to).getBaseFrameId());

            const Eigen::Vector3d p_M_fi =//就是把这个点的坐标从世界坐标系转到了当前任务的坐标系下
                    mission_baseframe_to.transformPointInGlobalFrameToMissionFrame(p_G_fi);
            const Eigen::Vector3d p_I_fi = T_I_M * p_M_fi;//然后再转到要合并的那个节点的body坐标系下

            //landmark_store_to储存的是这个节点第一次看到的路标点，所以另一个节点的第一次观测的路标点一定不在这个之内
            CHECK(!landmark_store_to.hasLandmark(landmark_id));
            landmark_store_to.addLandmark(landmark);//将被合并节点中的路标点首次观测信息加进合并的节点中
            vi_map::Landmark& new_landmark = landmark_store_to.getLandmark(landmark_id);
            new_landmark.set_p_B(pose::Position3D(p_I_fi));//因为节点换了，所以要更新第一次观测在节点中的位置信息

            CHECK_EQ(getLandmarkStoreVertexId(landmark_id), vertex_id_from);
            //landmark_index存的是这个路标点第一次被哪个节点观测到，所以也需要更新
            landmark_index.updateVertexOfLandmark(landmark_id, vertex_id_to);

            landmarks_to_be_removed.insert(landmark_id);//将这个被合并的节点中被首次观测到的地图点加到移除的set中
        }

        for (const vi_map::LandmarkId& landmark_id : landmarks_to_be_removed) {
            landmark_store_from.removeLandmark(landmark_id);//就是把被合并节点中的首次观测到的路标点进行清空
        }
    }

// Currently assumes vertex_from is the next after vertex_to.
    void VIMap::mergeNeighboringVertices(//就是将第二个节点合并进第一个节点
        ...
        vi_map::MissionId mission_id = getVertex(merge_into_vertex_id).getMissionId();//得到合并节点的任务id

        pose_graph::VertexId check_vertex_id;
        getNextVertex(
                merge_into_vertex_id, getGraphTraversalEdgeType(mission_id),
                &check_vertex_id);
        //因为要合并的肯定就是关键帧节点的后一个节点，这里再检查一下
        ...
        std::unordered_set<pose_graph::EdgeId> incoming, outgoing;//入边，出边
        vi_map::Vertex& next_vertex = getVertex(next_vertex_id);//通过id找到被合并的节点（最近关键帧节点的下一个节点）

        next_vertex.getOutgoingEdges(&outgoing);//得到被合并的节点连接的出边
        next_vertex.getIncomingEdges(&incoming);//得到被合并的节点连接的入边
        // It's possible that next vertex is the last vertex in the mission so
        // we need to handle no outgoing edge case.被合并的节点（最近关键帧节点的下一个节点）如果是最后一个节点时，是没有出边的
        ...
        // been deleted.
        //我们需要跟踪在以下循环中被删除的地标。当一个地标只能从一个顶点观察到，但是可以从这个顶点的多个帧中观察到，
        // 这就防止了崩溃。对于第一帧，本例中地标将从地图中删除。所有后续的帧也尝试访问这个地标，但由于它已被删除，所以将失败。
        std::unordered_set<LandmarkId> deleted_landmarks;//记录被删除过的地图点
        for (unsigned int frame_idx = 0u; frame_idx < num_frames; ++frame_idx)//遍历这个节点下的所有相机
        {
            if (!next_vertex.isVisualFrameSet(frame_idx)) {//判断是否有这帧
                continue;
            }
            const size_t num_of_keypoints =
                    next_vertex.observedLandmarkIdsSize(frame_idx);//这个节点中的这个相机所观测到的地图点的数量
            for (size_t i = 0u; i < num_of_keypoints; ++i) //遍历这个相机能看到的地图点
            {
                vi_map::LandmarkId landmark_id =
                        next_vertex.getObservedLandmarkId(frame_idx, i);

                if (landmark_id.isValid() && deleted_landmarks.count(landmark_id) == 0u) //如果这个路标点还没有被删除过
                {
                    vi_map::Landmark& landmark = getLandmark(landmark_id);

                    // Check for orphaned landmarks.
                    if (landmark.numberOfObserverVertices() == 1u)//如果只有这个节点看到过这个地图点，直接删除就好了
                    {
                        // Our merged vertex is the only vertex seeing this landmark.
                        removeLandmark(landmark_id);
                        deleted_landmarks.emplace(landmark_id);
                    } else {//如果不只这个节点看到过这个地图点,需要把这个地图点关于这个节点的观测信息全部去除
        ...
        // Remove the vertex.
        //将这个节点首次观测的地图点汇合到前一个关键帧节点，把这个节点连接的出入边都删掉，把地图点关于这个节点的观测信息都删除后
        // ，就可以正式的在位姿图里去掉这个节点
...
void VIMap::getAllVertexIdsInMissionAlongGraph(
  ...
  CHECK(hasMission(mission_id));
  vertices->clear();//这里节点为什么会清空？

  const vi_map::VIMission& mission = getMission(mission_id);//获得当前任务
  pose_graph::VertexId current_vertex_id = mission.getRootVertexId();//这里是返回任务的根节点
    ...
    const vi_map::MissionId& mission_id, pose_graph::EdgeIdList* edges) const
{
    ...
    const vi_map::VIMission& mission = getMission(mission_id);//得到任务的id
    pose_graph::VertexId current_vertex_id = mission.getRootVertexId();//得到任务的根节点
    ...
    pose_graph::EdgeIdList* edges) const
{
    ...
    const vi_map::VIMission& mission = getMission(mission_id);//得到当前任务
    pose_graph::VertexId current_vertex_id = mission.getRootVertexId();//当前任务的根节点
    ...
    pose_graph::EdgeIdList* edge_ids) const
{
    ...
    pose_graph::EdgeIdSet incoming_edges;//得到所有入边
    ...
    for (const pose_graph::EdgeId& edge_id : incoming_edges) {
        if (getEdgeType(edge_id) == ref_edge_type) {
            edge_ids->push_back(edge_id);//对应这个类型的边的id
        }
    }
}
