...
void serializeMissionsAndBaseframes(
    const vi_map::VIMap& map, vi_map::proto::VIMap* proto)
    {
        ...
        map.getAllMissionIds(&mission_ids);//得到所有的任务id_vec
        for (const MissionId& id : mission_ids)
        {
            id.serialize(proto->add_mission_ids());//保存所有的任务id
...
{
    const size_t num_vertices = map.numVertices();//地图所有节点的数量
    const size_t num_vertices_protos =//（200-1+节点数）/200,就是1个节点就需要一个文件夹
            (save_config.vertices_per_proto_file - 1u + num_vertices) /
            save_config.vertices_per_proto_file;
    return internal::kMinNumProtos + num_vertices_protos;//4个必要文件加节点需要的文件数量
}
...
{
    CHECK(function);
    const size_t num_protos = internal::numberOfProtos(map, save_config);//需要的文件夹的数目
    ...
    std::function<void(const std::vector<size_t>)> serialize_function =
            [&](const std::vector<size_t>& range)
            {
                ...
                for (size_t task_idx : range)
                {
                    ...
                    switch (task_idx)
                    {
                        //每一个地图都有自己的坐标系，这里的任务就是他们的坐标系，所有任务又都在同一个坐标系下（多地图的构建）
                        // Baseframe就是每个地图第一个节点。需要保存任务id，以及基准帧到世界坐标系的变换，为了统一到一个世界坐标系下
    ...
    const size_t num_threads =//最大是8线程，在8以下的线程中进行选择
            std::min(common::getNumHardwareThreads(), kMaxNumberOfThreads);
    //输入要创建的文件夹数目，序列化函数，并行选项，线程数量
...
bool saveMapToFolder(
    ...
    vi_map::VIMap* map)
    {
        std::string complete_folder_path;//完整的路径
        common::concatenateFolderAndFileName(//kFolderName[] = "vi_map" complete_folder_path = folder_path+getSubFolderName
        ...
        map->setMapFolder(folder_path);//创建map的文件夹,建立map和resource文件夹

        // Serialize the sensors.
        std::string sensors_yaml_filepath;
        common::concatenateFolderAndFileName(//在vi_map中生成传感器yaml文件
                complete_folder_path, internal::kYamlSensorsFilename,
                &sensors_yaml_filepath);
        map->getSensorManager().serializeToFile(sensors_yaml_filepath);//生成sensors.yaml文件


        /*
         * size_t serializeToFunction(
    const vi_map::VIMap& map, const backend::SaveConfig& save_config,
    const std::function<bool(const size_t, const proto::VIMap&)>&  // NOLINT,输入size_t,proto::VIMap返回bool
    function);
         */
