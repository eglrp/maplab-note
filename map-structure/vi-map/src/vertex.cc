...
Vertex::Vertex(
      ...
      observed_landmark_ids_(observed_landmark_ids)
    {
        ...
        checkConsistencyOfVisualObservationContainers();//size应该是一样大的，做了一个检测

        accel_bias_ = imu_ba_bw.head<3>();//设置bias
        gyro_bias_ = imu_ba_bw.tail<3>();
        v_M_.setZero();//当前节点的速度信息设成0,后面会再重新设置
...
void Vertex::checkConsistencyOfVisualObservationContainers() const //检查观测的连续性
{
  CHECK_EQ(n_frame_->getNumFrames(), observed_landmark_ids_.size());
  CHECK_EQ(n_frame_->getNumFrames(), n_frame_->getNumCameras());
  for (unsigned int frame_idx = 0; frame_idx < n_frame_->getNumFrames();
...
void Vertex::getFrameObservedLandmarkIds(
  ...
  landmark_ids->clear();
  *landmark_ids = observed_landmark_ids_[frame_idx];//得到当前这帧的所有观测点
}
