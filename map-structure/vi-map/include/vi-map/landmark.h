...
class Landmark {
 ...
 private:
  LandmarkId id_;//路标点id
  KeypointIdentifierList observations_;//存储了所有关于这个地图点的观测信息
  Quality quality_;//这个路标点的质量

  // Appearance vector, with one appearance per observation.
  std::vector<int> appearances_;//记录是否出现过

  // Position and covariance w.r.t. landmark baseframe. The covariance is
  // optional to reduce the memory usage.
  pose::Position3D B_position_;//这个路标点第一次被观测到时在body坐标系下的坐标
  AlignedUniquePtr<Eigen::Matrix3d> B_covariance_;//路标点的协方差
};
