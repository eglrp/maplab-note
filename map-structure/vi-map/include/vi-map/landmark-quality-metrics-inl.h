        ...
        const LandmarkWellConstrainedSettings& settings)
{
    ...
    if (num_observations < settings.min_observers) {//再检测一下最小观测数量
        return false;
    }

    if (signed_distance_to_closest_observer >//检查一下地图点和当前帧之间的距离
    ...
    double min_cos_angle = 1.0;//这里计算了两帧的光心射线之间的夹角
    ...
    double angle_deg = acos(min_cos_angle) * kRadToDeg;//最小夹角转换成度的形式
    //就是说任意两帧观测这个地图点所夹成的最小夹角都要大于这个角度才认为多视图三角化是准的
