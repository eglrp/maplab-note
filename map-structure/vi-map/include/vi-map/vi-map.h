...
class VIMap : public backend::ResourceMap,
 ...
 public:
  ...
  inline Eigen::Quaterniond getVertex_G_q_I(
    ...
    inline void setVertex_T_M_I(const pose_graph::VertexId& vertex_id,const Eigen::Quaterniond Q_M_I,const Eigen::Vector3d t_M_I);
  ...
  /// Get all the vertex ids in the order that they appear when traversing the//便利
 ...
 private:
  ...
  PoseGraph posegraph;
  VIMissionMap missions;//所有任务的map，方便查找
  //typedef AlignedUnorderedMap<MissionBaseFrameId, MissionBaseFrame> MissionBaseFrameMap;
  //也就是储存所有基准帧信息的map
  MissionBaseFrameMap mission_base_frames;
  LandmarkIndex landmark_index;//负责储存地图点第一次被哪个节点看到的信息
  SensorManager sensor_manager_;//相机传感器信息
  OptionalSensorDataMap optional_sensor_data_map_;//其他传感器，比如imu，GPS等信息
