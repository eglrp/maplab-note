...
class Vertex : public pose_graph::Vertex {
 ...
 private:
  ...
  aslam::Transformation T_M_I_;//Tmi是当前mission坐标系下body坐标系的位姿
  Eigen::Vector3d v_M_;//当前节点的速度信息
  Eigen::Vector3d accel_bias_;//加速度bias
  Eigen::Vector3d gyro_bias_;//陀螺仪bias

  // Incoming, outgoing edges.
  pose_graph::EdgeIdSet incoming_edges_;//所有的入边
  pose_graph::EdgeIdSet outgoing_edges_;//这个节点所有的出边

  aslam::VisualNFrame::Ptr n_frame_;//存储观测信息
  std::vector<LandmarkIdList> observed_landmark_ids_;
  //存储的是观测到的地图点信息,vector的索引是多相机的id，比如只有n相机时，size就是n
  //observed_landmark_ids_[frame_idx][keypoint_idx] 就是特征点对应的地图点的id

  // Landmark storage.
  LandmarkStore landmarks_;//储存的是由这个节点第一次观测到的地图点的信息
...
/*
    class Vertex : public pose_graph::Vertex
    {//节点类的主要成员变量
        pose_graph::VertexId id_;//当前这个节点的id
        vi_map::MissionId mission_id_;//当前任务坐标系的id

        // Pose, velocity, IMU biases.
        aslam::Transformation T_M_I_;//当前mission坐标系下body坐标系的位姿
        Eigen::Vector3d v_M_;//当前节点的速度信息
        Eigen::Vector3d accel_bias_;//加速度bias
        Eigen::Vector3d gyro_bias_;//陀螺仪bias

        // Incoming, outgoing edges.
        pose_graph::EdgeIdSet incoming_edges_;//这个节点所有的入边（vio模式下只有一出一入）
        pose_graph::EdgeIdSet outgoing_edges_;//这个节点所有的出边
        ...
        // Landmark storage.
        LandmarkStore landmarks_;//里面存储的是首次是被这个节点所观测到的地图点
