...
vi_map::MissionBaseFrame& VIMap::getMissionBaseFrameForVertex(
  ...
  const vi_map::VIMission& mission = getMissionForVertex(id);//得到当前节点属于哪一个任务
  //mission.getBaseFrameId()找到当前任务的基准帧id，再从地图里的基准帧序列里找到属于这个任务的基准帧，
  // 基准帧数据结构里存储了这个任务到世界坐标系的变换
  return getMissionBaseFrame(mission.getBaseFrameId());
}
...
inline void VIMap::setVertex_T_M_I(const pose_graph::VertexId& vertex_id,const Eigen::Quaterniond Q_M_I,const Eigen::Vector3d t_M_I)
{
    vi_map::Vertex* vertex = getVertexPtr(vertex_id);
    vertex ->set_q_M_I(Q_M_I);
    vertex ->set_p_M_I(t_M_I);
}
...
void VIMap::addVertex(vi_map::Vertex::UniquePtr vertex_ptr) {
  ...
  posegraph.addVertex(std::move(vertex_ptr));//实际就是把节点id和节点本身存储进去std::unordered_map<VertexId, AlignedUniquePtr<Vertex>> VertexMap
}
...
pose_graph::Edge::EdgeType VIMap::getEdgeType(
  ...
  return posegraph.getEdgePtr(edge_id)->getType();
    ...
    inline bool VIMap::getNextVertex(//得到这个边连的另一个节点
            ...
            pose_graph::VertexId* next_vertex_id) const
    {
        ...
        getVertex(current_vertex_id).getOutgoingEdges(&outgoing_edges);//得到这个节点的所有出边
        ...
        for (const pose_graph::EdgeId& edge_id : outgoing_edges)
        {
            ...
            if (edge->getType() == edge_type) {
                *next_vertex_id = edge->to();
                return true;//边的类型也要对应上，然后边连接的节点就是下一个节点
            }
        }
        return false;
    }
