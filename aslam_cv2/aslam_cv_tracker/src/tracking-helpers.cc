...
//将cv提取出的关键点和描述子放到视觉帧的channels_中,  aslam::channels::ChannelGroup channels_是可以用来存储各种数据的，比如这帧的描述子，观测，等等
            ...
            const double fixed_keypoint_uncertainty_px, aslam::VisualFrame* frame)
    {
        ...
        const size_t kNumNewKeypoints = new_cv_keypoints.size();//总共有多少个特征点
        ...
        {
            const cv::KeyPoint& cv_keypoint = new_cv_keypoints[idx];//特征点
            new_keypoints_measurements.col(idx)(0) = static_cast<double>(cv_keypoint.pt.x);//特征点的测量
            new_keypoints_measurements.col(idx)(1) = static_cast<double>(cv_keypoint.pt.y);
            new_keypoint_scores(idx) = static_cast<double>(cv_keypoint.response);//特征点的得分，该点角点的程度
            new_keypoint_scales(idx) = static_cast<double>(cv_keypoint.size);//特征点的大小
            new_keypoint_orientations(idx) = static_cast<double>(cv_keypoint.angle);//特征点的方向
        }


        //就是将特征点的像素坐标，得分，特征点的大小，方向放进aslam::channels::ChannelGroup channels_中
        ...
        frame->swapTrackIds(&track_ids);//设置跟踪id

        frame->setDescriptors(//设置描述子
