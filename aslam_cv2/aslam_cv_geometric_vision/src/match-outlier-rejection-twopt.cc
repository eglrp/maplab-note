...
//对前后的匹配进行ransac处理
    //输入当前帧，前一帧，他们两个之间的旋转，匹配的得分，false
bool rejectOutlierFeatureMatchesTranslationRotationSAC(
  ...
  aslam::FrameToFrameMatches matches_without_score_kp1_k;//匹配索引
  ...
  using opengv::relative_pose::CentralRelativeAdapter;//初始化旋转的相对关系类
  CentralRelativeAdapter adapter(bearing_vectors_kp1, bearing_vectors_k,//输入的是当前的特征点的归一化坐标，之前帧的归一化坐标
                                 q_Ckp1_Ck.getRotationMatrix());

  typedef opengv::sac_problems::relative_pose::RotationOnlySacProblem
      RotationOnlySacProblem;
  std::shared_ptr<RotationOnlySacProblem> rotation_sac_problem(//opengv会给所有匹配分配独立的索引
