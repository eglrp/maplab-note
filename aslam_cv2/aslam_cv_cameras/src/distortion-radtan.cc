    ...
    Eigen::Matrix2d* out_jacobian) const
{
    ...
    double my2_u = y * y;
    double mxy_u = x * y;
    double rho2_u = mx2_u + my2_u;//r
    double rad_dist_u = k1 * rho2_u + k2 * rho2_u * rho2_u;

    if (out_jacobian)
    {//这个雅克比是去畸变的归一化坐标对于带畸变的归一化的雅克比
    ...
    x += x * rad_dist_u + 2.0 * p1 * mxy_u + p2 * (rho2_u + 2.0 * mx2_u);//对归一化坐标去畸变
    y += y * rad_dist_u + 2.0 * p2 * mxy_u + p1 * (rho2_u + 2.0 * my2_u);
}
//对归一化平面上的点进行畸变矫正
//x修正 = x(1+k1*r2 +k2*r4)+2p1xy+p2(r2+2x2)
//y修正 = y(1+k1*r2 +k2*r4)+2p2xy+p1(r2+2y2)
//这里求得就是各个畸变参数对于归一化点的扰动

// x/z = (x/z)(1+k1*r2/z2 +k2*r4/z4)+2p1xy/z2 + p2(r2+2x2)/z2
// y/z = (y/z)(1+k1*r2/z2 +k2*r4/z4)+2p2xy/z2 + p1(r2+2y2)/z2
