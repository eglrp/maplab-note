...
NCamera::NCamera(const NCameraId& id, const TransformationVector& T_C_B,
    ...
    : id_(id),//
      T_C_B_(T_C_B),//多个相机位姿
      cameras_(cameras),//多个相机
      label_(label)  //这个多相机系统的标签
      {
  CHECK(id.isValid());
  initInternal();
}
...
NCamera::Ptr NCamera::loadFromYaml(const std::string& yaml_file) {//在前端初始化时会调用读yaml文件的操作
  try {
...
NCamera::Ptr NCamera::deserializeFromYaml(const YAML::Node& yaml_node) {//node里存的是相机的各种
  try {
    return yaml_node.as<aslam::NCamera::Ptr>();//初始化aslam::NCamera
  } catch (const std::exception& ex) {
...
void NCamera::initInternal() {
  ...
  for (size_t i = 0; i < cameras_.size(); ++i) {
    ...
    id_to_index_[cameras_[i]->getId()] = i;//配置索引，就是这个id是第几个相机
  }
}
