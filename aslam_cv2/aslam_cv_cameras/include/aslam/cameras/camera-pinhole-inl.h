    ...
    const Eigen::MatrixBase<DerivedPoint3d>& point_3d) const
{
    ...
    const bool visibility = isKeypointVisible(kp);//先判断这个重投影以后的特征点是不是在图像里

    if (visibility && (point_3d[2] > kMinimumDepth))//在图像内，且3d点的深度大于0，就是可见的
        return ProjectionResult(ProjectionResult::Status::KEYPOINT_VISIBLE);
    else if (!visibility && (point_3d[2] > kMinimumDepth))//不在图像内，且3d点深度大于0
        return ProjectionResult(ProjectionResult::Status::KEYPOINT_OUTSIDE_IMAGE_BOX);
    else if (point_3d[2] < 0.0)//3d点深度小于0
