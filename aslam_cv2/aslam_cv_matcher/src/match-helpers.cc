...
//输入前一帧，qbkb1，输出的是预测的下一帧的特征点，以及是否真的预测上了
    void predictKeypointsByRotation(const VisualFrame& frame_k,
        ...
        // Early exit for identity rotation.旋转太小的话，提前退出
        ...
        Eigen::Matrix3Xd bearing_vectors_k;//像素坐标投影到相机坐标系
        //prediction_success在这里是每个点是否投影成功
        ...
        // Rotate the bearing vectors into the frame_kp1 coordinates.
        //因为q是qbkb1,也就是q当前帧，之前帧的旋转变换，所以是对之前这帧的相机坐标系下的位置做了一个旋转，对于向量的旋转当然是左乘了
        //所以bearing_vectors_kp1就是假设的在当前帧中这些特征点的位置
        ...
        //predicted_keypoints_kp1就是再投影回去的像素坐标，就是先将前一帧的像素坐标投到相机坐标系下，旋转，再投影回去
        ...
        //根据反投影和投影结果设置成功，输出失败预测的初始未旋转关键点。
        const Eigen::Matrix2Xd& keypoints_k = frame_k.getKeypointMeasurements();//前一帧的所有特征点的像素坐标
        CHECK_EQ(keypoints_k.cols(), predicted_keypoints_kp1->cols());

        for (size_t idx = 0u; idx < projection_results.size(); ++idx) //遍历投影结果
        {

            (*prediction_success)[idx] = (*prediction_success)[idx] &&
                                         projection_results[idx].isKeypointVisible();//必须要投影过去和投影回来都成功才可以

            // Set the initial keypoint location for failed predictions.
            //用imu的陀螺仪来预测位置如果没成功的话，那么直接用之前的像素坐标来作为预测值
