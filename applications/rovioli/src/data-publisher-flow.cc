...
void DataPublisherFlow::attachToMessageFlow(message_flow::MessageFlow* flow)
{
  ...
  flow->registerSubscriber<message_flow_topics::LOCALIZATION_RESULT>(
      ...
      [&](const vio::LocalizationResult::ConstPtr& localization) {
        ...
        if(FLAGS_save_pose)
        {
            localizationCallback(localization->timestamp,localization->T_G_I_lc_pnp);
        }
      });
    ...
    const aslam::Transformation& T_G_M)
{
    ...
    if (has_T_G_M) {
        aslam::Transformation T_G_I = T_G_M * T_M_I;//这个是在世界坐标系中的位姿
        visualization::Sphere sphere;
...
void DataPublisherFlow::localizationCallback(
  ...
  visualization::publishSpheres(
    ...
    void DataPublisherFlow::localizationCallback(
            int64_t timestamp_ns,const aslam::Transformation& T_G_M)
    {
        std::ofstream foutC("/home/wya/maplab_localize.txt", std::ios::app);
        foutC.setf(std::ios::fixed, std::ios::floatfield);
        foutC.precision(6);
        foutC<<aslam::time::nanoSecondsToSeconds(timestamp_ns)<<" "<<T_G_M.getPosition()[0]
             <<" "<<T_G_M.getPosition()[1]<<" "<<T_G_M.getPosition()[2]<<" "<<T_G_M.getRotation().x()
             <<" "<<T_G_M.getRotation().y()<<" "<<T_G_M.getRotation().z()<<" "<<T_G_M.getRotation().w()<<std::endl;
    }
