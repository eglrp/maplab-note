...
struct RovioBuilder {
  ...
  static constexpr int kMaxNumFeatures = 45;
  ...
  rovio::RovioInterface* operator()(
      const rovio::FilterConfiguration& filter_config,//配置文件，多相机的标定
...
void convertAslamToRovioCamera(//由ncam结构转成相机标定vector，其实就是数据类型的转换
    ...
    //typedef Aligned<std::vector, CameraCalibration> CameraCalibrationVector;
{
    ...
    const size_t num_cameras = aslam_cameras.numCameras();//相机数量
    for (size_t cam_idx = 0u; cam_idx < num_cameras; ++cam_idx) //遍历所有相机，事实上在vio部分就只有一个相机
    {
        ...
        switch (aslam_camera.getDistortion().getType()) //4种畸变模型
        {
        ...
        const Eigen::Vector3d MrMC = T_C_B.getPosition();//在相机坐标系中body系的位置
        const kindr::RotationQuaternionPD qCM(
                T_C_B.getRotation().toImplementation());//在相机坐标系中body系的姿态
        rovio_camera.setCameraExtrinsics(MrMC, qCM);//设置外参
...
rovio::RovioInterface* constructAndConfigureRovio(
    ...
    const vi_map::ImuSigmas& imu_config)
    {
        ...
        size_t num_cameras = camera_calibration.getNumCameras();//有多少个相机，这里默认就是一个相机
        rovio::CameraCalibrationVector rovio_calibrations;
        //typeef Aligned<std::vector, CameraCalibration> CameraCalibrationVector;

        //将camera_calibration转换成rovio_calibrations数据类型
        ...
        const std::string rovio_config_template_file =//读取配置文件
        ...
        //rovio_configuration里的参数配置，协方差用给rovio的那个yaml文件初始化
        initFilterConfigurationFromGFLags(&rovio_configuration);//设置可视化
        setRovioImuSigmas(imu_config, &rovio_configuration);

        rovio::RovioInterface* rovio = nullptr;
        switch (num_cameras) {//定位模式只支持两个,初始化rovio内核
            case 1:
