...
DataSource* createAndConfigureDataSourcefromGFlags(//初始化topic相关话题
    const aslam::NCamera& camera_system, const vi_map::Imu& imu_sensor)
{
    const vio_common::RosTopicSettings topic_settings(camera_system, imu_sensor);//初始化imu等传感器话题名，相机的话题名
    ...
    switch (source_type)
    {
        case DataSourceType::kRosBag:
            ...
            break;
        case DataSourceType::kRosTopic://我们一般是利用的rostopic，所以用这种初始化方式
