...
inline vio::ImuMeasurement::Ptr convertRosImuToMaplabImu(
  ...
  imu_measurement->imu_data << imu_msg->linear_acceleration.x,//当前imu数据
...
inline vio::ImageMeasurement::Ptr convertRosImageToMaplabImage(//将接收到的图片ros消息转成需要的数据结构
...
//    };
    ...
    image_measurement->image = cv_ptr->image.clone();//克隆图片
    image_measurement->timestamp =//转换成拿秒
            rosTimeToNanoseconds(image_message->header.stamp);
    image_measurement->camera_index = camera_idx;//第几个相机的参数？
