...
//具体的JPL四元数转旋转矩阵可以看MARS lab的Indirect Kalman Filter for 3D Attitude Estimation
  (*rot_matrix)(0, 0) = one - two * (q(1) * q(1) + q(2) * q(2));//1-2q2^2-2q3^2
...
void signedQuaternionProductJPL(
  ...
  common::skew(q1.head(3), skew_q1);//构造JPL形式的左乘四元数算子
