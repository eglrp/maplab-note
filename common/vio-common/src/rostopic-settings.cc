...
RosTopicSettings::RosTopicSettings(
    const aslam::NCamera& camera_system, const vi_map::Imu& imu)//输入多相机系统,imu
    : imu_topic(imu.getHardwareId()),//hardware_id是存的话题名字
      camera_topic_suffix(FLAGS_vio_camera_topic_suffix),//相机话题的后缀
      absolute_pose_topic(""),//这里程序并没有输入，不过留了这个接口
      hardware_features_topic(FLAGS_vio_hardware_features_topic),//这个是什么接口？
      gps_wgs_topic(""),//
      gps_utm_topic("")
{
    ...
    for (size_t cam_idx = 0u; cam_idx < camera_system.getNumCameras();
         ++cam_idx)
    {//遍历相机系统，然后camera_topic_cam_index_map存储{话题名,相机id}
        camera_topic_cam_index_map.emplace(
                std::make_pair(//话题名，相机id
