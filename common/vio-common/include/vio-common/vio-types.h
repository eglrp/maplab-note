...
struct LocalizationResult {
  ...
  int64_t timestamp;//当前找到闭环的这个多相机系统中最小的那个相机相机时间戳，单位纳秒
  aslam::NFramesId nframe_id;//多相机的id
  aslam::Transformation T_G_I_lc_pnp;//pnp以后又已知相机和imu的外参，求出的body坐标系在世界坐标系中的位姿

  enum class LocalizationMode { kGlobal, kMapTracking };
  LocalizationMode localization_type;//这里因为基于位姿先验的定位还没有实现，所以只有kGlobal
};
...
struct ImageMeasurement
{
    ...
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    int64_t timestamp;//这个相机得到这个图片时的时间戳，这里的时间是纳秒
    int camera_index;//多相机系统中这个相机的id
    cv::Mat image;//这个相机在timestamp时得到的图片
...
struct ImuMeasurement {
  ...
  int64_t timestamp;//单位是纳秒
  ImuData imu_data;//前3维加速度，后3维是角速度
};
...
struct SynchronizedNFrameImu {
 public:
  ...
  //第一个imu数据是上一组imu数据的最后一个
  Eigen::Matrix<int64_t, 1, Eigen::Dynamic> imu_timestamps;//imu在k多相机时刻和k+1多相机时刻之间的时间戳
  Eigen::Matrix<double, 6, Eigen::Dynamic> imu_measurements;//imu的测量，间隔同上

  aslam::VisualNFrame::Ptr nframe;//N相机系统，存了每个相机的图片，时间戳。
  // 在racker_flow_步骤中还会对特征点的像素坐标和描述子，点管理等信息进行更新
