...
namespace message_flow {
    ...
    //就比如<message_flow_topics::IMAGE_MEASUREMENTS>
    /*
     * 由MESSAGE_FLOW_TOPIC(IMAGE_MEASUREMENTS, vio::ImageMeasurement::Ptr)构建
// struct IMAGE_MEASUREMENTS里kMessageTopic就是IMAGE_MEASUREMENTS，message_type就是vio::ImageMeasurement::Ptr
     */
template <typename MessageTopicDefinition>//建立消息接收器
PublisherFunction<MessageTopicDefinition> MessageFlow::registerPublisher()
{
  typedef typename MessageTopicDefinition::message_type MessageType;//比如vio::ImageMeasurement::Ptr
    ...
    const SubscriberCallback<MessageTopicDefinition>& callback)
{
    ...
    CHECK(callback);
    {
        ...
        // to.如果需要，为节点分配新的订阅者队列。回调函数定义将此队列的所有消息传递到的函数。
        ...
        //还将新订阅服务器添加到发布/子网络定义中。回调函数定义消息发布时执行的逻辑。在本例中，我们将其添加到交付队列中，然后dispatcher根据其策略发送消息。
