...
bool JplQuaternionParameterization::Plus(
  ...
  Eigen::Vector4d q_delta;//这里的扰动是roll，pitch，yaw都进行了扰动
...
bool JplYawQuaternionParameterization::Plus(
  ...
  Eigen::Vector4d q_AhatA_delta;//这是一个微小的扰动，因为只是对yaw进行扰动，所以是对z轴进行了扰动
  ...
  common::positiveQuaternionProductJPL(q_AhatA_delta, q_AB, q_AB_plus_delta);//因为是JPL形式，所以是q_AhatA_delta左乘q_AB
  return true;
}
...
JplRollPitchQuaternionParameterization::JplRollPitchQuaternionParameterization(
  ...
  common::toRotationMatrixJPL(common::quaternionInverseJPL(q_GM_JPL), &R_M_G_);//这里不应该是R_G_M么，我怎么感觉这里都写反了
}
