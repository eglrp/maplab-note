...
struct ProblemInformation
{
    ...
    void addResidualBlock(
            ResidualType residual_type,//残差类型
            std::shared_ptr<ceres::CostFunction> cost_function,//残差类型对应的costfun
            std::shared_ptr<ceres::LossFunction> loss_function,//核函数
            const std::vector<double*>& parameter_blocks)
    {//要优化的参数
        ResidualInformation residual_info(//初始化残差信息
        ...
        active_parameter_blocks.insert(//待优化的参数块，都先设成活动的，之后再决定谁要fix
    ...
    ResidualInformationMap residual_blocks;//map,key是代价函数，value是ResidualInformation，里面是costfun，鲁棒核函数等等

    std::unordered_set<double*> active_parameter_blocks;//要优化的参数中不fix的部分
    std::unordered_set<double*> constant_parameter_blocks;//fix的部分
    ...
    // the default group.
    //与每个参数块关联的组类型。这是特殊的求解器所需要的，它以不同的方式处理某些参数组。-1是默认组。
