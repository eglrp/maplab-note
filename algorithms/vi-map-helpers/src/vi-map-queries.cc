    ...
    action) const
{
    ...
    map_.getAllVertexIdsInMission(mission_id, &all_mission_vertices);//得到这个任务的所有节点
    for (const pose_graph::VertexId& vertex_id : all_mission_vertices)
    {//遍历所有节点观测到的地图点，并且将其添加到observed_landmarks
...
void VIMapQueries::getLandmarksObservedByMission(//将这个任务下的所有地图点id都加到set中
...
//返回的是这个节点所看到的地图点，经过质量评判后，得到的好点的数量
    ...
    const vi_map::Vertex& vertex, const size_t frame_index) const
{
    ...
    vertex.getFrameObservedLandmarkIds(frame_index, &landmark_ids);//得到这帧看到的所有地图点id

    size_t well_constrained_landmarks = 0;
    for (const vi_map::LandmarkId landmark_id : landmark_ids)//遍历所有地图点id
    {
        if (landmark_id.isValid())
        {
            //进行地图点质量的判定
            //1检测一下最小观测数量,2检查一下地图点和当前帧之间的距离3计算了两帧的光心射线之间的夹角
...
//        required, better use: getNumberOfCommonLandmarks(v1, v2)
            ...
            vi_map::LandmarkIdSet* landmarks) const
    {
        ...
        vi_map::LandmarkIdList vector_landmark_ids_1;//节点1观测到的所有的地图点id的集合
        map_.getVertex(vertex_1_id).getAllObservedLandmarkIds(&vector_landmark_ids_1);
        vi_map::LandmarkIdSet landmark_ids_1(vector_landmark_ids_1.begin(),//节点1观测到的所有的地图点id的set形式
        ...
        vi_map::LandmarkIdList landmark_ids_2;//节点2观测到的所有的地图点id的集合
        ...
        for (const vi_map::LandmarkId& landmark2_id : landmark_ids_2)
        {
            if (landmark2_id.isValid()) {
                ...
                {//如果节点1也看到了节点2看到的地图点，计数+1
                    ++result;
                    if (landmarks) //如果landmarks不是null，则添加进去
                    {
