...
//开始relax地图
bool VIMapRelaxation::relax(
    const ceres::Solver::Options& solver_options,//ceres优化参数
    const vi_map::MissionIdList& mission_id_list, vi_map::VIMap* map) {//地图
  CHECK_NOTNULL(map);

  vi_map::MissionIdSet mission_ids(//所有任务集合
      mission_id_list.begin(), mission_id_list.end());

  detectLoopclosures(mission_ids, map);//检测闭环
