...
void OptimizationStateBuffer::copyAllStatesBackToMap(vi_map::VIMap* map) const {//优化以后再将所有状态量拷贝回map
  //就是把JPL格式的转成Hamilton格式的
...
void OptimizationStateBuffer::importStatesOfMissions(
  ...
  importKeyframePosesOfMissions(map, mission_ids);//将所有节点的位姿存到vertex_q_IM__M_p_MI_里，其中q是JPL格式
  importBaseframePoseOfMissions(map, mission_ids);//将所有基准帧存到baseframe_q_GM__G_p_GM_里，其中q是JPL格式
  importCameraCalibrationsOfMissions(map, mission_ids);//将每个任务中的相机系统的外参存到camera_q_CI__C_p_CI_中，其中q是JPL格式
  importSensorCalibrationsOfMissions(map, mission_ids);//这个vio系统貌似用不上，就是别的传感器，然后保存一下标定的外残
}
    ...
    const vi_map::VIMap& map, const vi_map::MissionIdSet& mission_ids)
{
    ...
    //遍历所有的任务，得到任务的所有节点，all_vertices是所有任务的节点总和
    ...
    for (const pose_graph::VertexId& vertex_id : all_vertices) //遍历所有的节点
    {
        const vi_map::Vertex& ba_vertex = map.getVertex(vertex_id);

        Eigen::Quaterniond q_M_I = ba_vertex.get_q_M_I();//节点在当前任务坐标系下的本体朝向
        ensurePositiveQuaternion(q_M_I.coeffs());//q.w需要是正的
        assertValidQuaternion(q_M_I);
//四元数形式推导vio误差一般有两种形式，Hamiltonian和JPL，比如vins就是用的Hamiltonian（Q_M_I），而是s-msckf用的是JPL(Q_I_M)，

        // Convert active Hamiltonian rotation (minkindr, Eigen) to passive JPL
        // which the error terms use. No inverse is required.
        //JPL第3维是w，用eigen的这个直接导过去就可以，这里的q_M_I和vertex_q_IM__M_p_MI_中的q是一样的，
        // 之所以下标表示不一样是因为转成R时的方法是相反的，所以转成R以后，就变成了R_IM,JPL格式的都是这样，下面对于JPL的转换也是同理
    ...
    const vi_map::VIMap& map, const vi_map::MissionIdSet& mission_ids)
{
    ...
    for (const vi_map::MissionId& mission_id : mission_ids)
    {//假如有三个任务，分别是单目，双目，四目，那么这里的ncameras就是1+2+4=7个相机，就是设置imu和相机之间的外参
