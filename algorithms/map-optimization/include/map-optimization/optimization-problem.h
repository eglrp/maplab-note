...
class OptimizationProblem {
 public:
  ...
  struct ProblemBookkeeping
    ...
    std::unordered_set<pose_graph::VertexId> keyframes_in_problem;//需要优化的节点id
 ...
 private:
  ...
  //增加簿记信息，以逐步更新内容。这必须与问题信息保持同步。就是用来记录要优化的节点id，地图点id
  ProblemBookkeeping problem_books_;

  // Copies of the map states on which the optimization will operate.
  OptimizationStateBuffer state_buffer_;//存储了要优化的节点位姿，基准帧位姿，相机外参等
