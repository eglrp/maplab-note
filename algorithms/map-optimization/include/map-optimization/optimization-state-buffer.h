...
class OptimizationStateBuffer {
 ...
 private:
  ...
  Eigen::Matrix<double, 7, Eigen::Dynamic> vertex_q_IM__M_p_MI_;//存储所有节点的位姿（JPL格式的四元数）
  ...
  std::unordered_map<aslam::CameraId, std::vector<aslam::NCameraId>>
      camera_id_to_ncamera_id_;//存的是当前这个相机属于哪一个相机系统
  std::unordered_map<aslam::CameraId, size_t> camera_id_to_camera_idx_;//相机的id到它所在的camera_q_CI__C_p_CI_的索引
