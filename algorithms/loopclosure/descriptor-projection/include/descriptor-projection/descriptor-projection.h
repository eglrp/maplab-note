...
struct ProjectedImage {
  ...
  int64_t timestamp_nanoseconds;//这帧的时间戳
  KeyframeId keyframe_id;//typedef vi_map::VisualFrameIdentifier KeyframeId;,这帧是在哪个节点的哪个相机被看到的
    DatasetId dataset_id;//这个帧所在的任务id
  Eigen::MatrixXf projected_descriptors;//所有有效的特征点对应的描述子投影以后的描述子
  Eigen::Matrix2Xd measurements;//所有的特征点对应的像素坐标
  std::vector<PointLandmarkId> landmarks;//地标点的索引
