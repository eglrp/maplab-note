        ...
        keypoint_to_best_structure_match)
{
    ...
    for (size_t inlier_idx = 0u; inlier_idx < inliers.size(); ++inlier_idx) //遍历所有内点
    {
        const int inlier_index = inliers[inlier_idx];//内点的索引，索引到structure_matches，代表的是哪一个匹配
        const double reprojection_error = inlier_distances_to_model[inlier_idx];//它的重投影误差
        const size_t frame_index =//哪个相机看到的
                structure_matches[inlier_index].frame_index_query;
        CHECK_LT(frame_index, query_vertex_n_frame.getNumFrames());
        const size_t keypoint_index =//关键点的索引
