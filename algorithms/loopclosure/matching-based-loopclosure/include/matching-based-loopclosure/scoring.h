...
// in most scenarios.
//只需为具有匹配数的ID获得投票数/匹配数。这是最快和最简单的评分函数，在大多数情况下都能很好地工作。
template <typename ScoreIdType>
void computeAccumulationScore(
    ...
    size_t /*num_descriptors_in_database*/, ScoreList<ScoreIdType>* scores)
    {
        ...
        const size_t num_ids = id_to_matches_map.size();//这个是所有候选描述子所在的帧的总数
        if (num_ids == 0u) {
                    ...
                    id_matches_pair : id_to_matches_map)
        {
            ...
            scores->emplace_back(id_matches_pair.first, score);//这个其实就是这帧中候选描述子数量的多少
        }
    };
