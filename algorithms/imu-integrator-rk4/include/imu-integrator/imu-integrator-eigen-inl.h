...
void ImuIntegratorEigen::integrate(
  ...
  const ScalarType o5 = static_cast<ScalarType>(0.5);//o5是0.5

  next_state->setZero();

  Eigen::Matrix<ScalarType, kImuReadingSize, 1> imu_reading;
  interpolateImuReadings(//相邻两次时刻k到k+1的加速度和速度数据的标称值，dt，0.5dt，返回插值的imu数据
...
//输入相邻两次时刻k到k+1的加速度和速度数据的标称值，dt，0.5dt，返回插值的imu测量值
template <typename ScalarType>
void ImuIntegratorEigen::interpolateImuReadings(
    ...
    const
    {
        ...
        *interpolated_imu_readings =//对imu进行插值
                imu_readings.template block<kImuReadingSize, 1>(0, 0) +
